...
由于真实数据尚未到位，使用 `dcma` 库中四张模拟表，一台设备一张表：
...
| G120电机4 | real_data_04 | 2026-07-19 16:00:00 ~ 16:19:56（300 行） | 全程正常（代码恒为 0），用于验证“未发现显式故障”路径 |

`real_data`（默认表）保存的是真实采集数据（曾出现 F1030），当前没有设备路由到它；
四台模拟设备全部使用专属表。
...
fault:
  diagnosis:
    allowed-assets: [G120电机1, G120电机2, G120电机3, G120电机4]
    ...
    device-telemetry-tables:            # 设备 -> 专属表路由（键含中文必须加方括号）
      ...
      "[G120电机4]": real_data_04
    latest-data-fallback-enabled: true  # 请求窗口无数据时回退到最近可用数据
    nominal-sampling-seconds: 4         # 模拟数据约 3~5 秒一条
```

### 实时模拟（telemetry_simulator.py）

历史模拟数据是固定的（2026-07-19），"现在有没有故障"这类实时提问在窗口内查不到数据，
只能得到"当前状态无法确认"的历史回退回答。为了在真实数据到位前验证实时链路，
提供了常驻遥测模拟器 `docs/script/telemetry_simulator.py`：

- 每 `nominal-sampling-seconds`（4 秒）为每台设备写入一条**当前时间**的遥测，
  使"现在/最近 N 分钟"的提问命中真实窗口，不再触发历史回退；
- 按剧本周期性注入异常段：G120电机1 每 30 分钟注入 2 分钟 A07089 报警；
  G120电机2 每 40 分钟注入 2 分钟 F30899 故障；G120电机3 每 50 分钟注入 2 分钟
  F07016 故障（相位互相错开）；G120电机4 始终正常；
- 调度基于 epoch 无状态计算，脚本重启不丢剧本；写入遵循上文的数据契约。

启动与停止：

```bash
python3 docs/script/telemetry_simulator.py --seed   # 一次性回填 real_data_04 历史数据
setsid nohup python3 docs/script/telemetry_simulator.py > telemetry-sim.log 2>&1 &
```

注意：模拟器刚启动的前几分钟，最近窗口内的数据完整率不足 `completeness-threshold`（0.8），
诊断会如实返回"数据不足"；持续运行约 5 分钟后窗口被填满，即返回正常的实时结论。
真实数据接入后停止该脚本即可，代码无需改动。

### 最新数据回退

若请求窗口仍然查不到任何数据（例如模拟器未运行、真实数据断流），
`TelemetryQueryService` 会自动改用该设备**最近可用**的数据窗口（以表内最新
`create_time` 为终点、向前推 `default-window-minutes`，并不早于表内最早记录）
重新查询，并在回答中明确"当前状态无法确认"。

回退发生时：

- 结果标记 `fallbackToLatestData=true`；
- 回答结论第一句话为"当前状态无法确认"，并注明设备最新数据时间（属于历史数据）；
- "最近一次观测"显示真实分析的数据范围，"结论边界"注明已回退至最近可用数据；
- 证据链（fd_evidence 的遥测证据摘要）也会记录回退标记、实际窗口与最后观测时间。
...
历史窗口（验证代码分类、报警措辞与可读证据）：

- `G120电机3在2026-07-19 12:30到12:40之间有什么故障？` —— 指定窗口命中 F07016；
- `诊断一下G120电机1在2026-07-19 14:50到15:05的情况` —— 命中 A07089 报警段，只能称为报警。

实时数据（需先启动 `telemetry_simulator.py` 约 5 分钟以上）：

- `G120电机4现在有没有故障？` —— 命中实时窗口，回答"未发现显式故障码或报警码。截至 {最近时间}"；
- `G120电机2最近10分钟有没有故障？` —— 视注入相位观测到 F30899 故障段或正常。

模拟器未运行时，实时提问会回退到历史数据，回答以"当前状态无法确认"开头。
